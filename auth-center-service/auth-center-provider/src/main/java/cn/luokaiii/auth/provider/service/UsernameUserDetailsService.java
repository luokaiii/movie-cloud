...
import cn.luokaiii.user.client.BaseUserService;
...
public class UsernameUserDetailsService implements UserDetailsService {
    ...
    @Autowired
    public UsernameUserDetailsService(BaseUserService baseUserService, RedisTemplate<String, String> roleTemplate) {
    ...
    public UserDetails loadUserByUsername(String s) throws UsernameNotFoundException {
        ...
        ResponseData<BaseUser> userData = baseUserService.getUserByUsername(s);
        ...
        BaseUser user = userData.getData();
    ...
    private List<GrantedAuthority> grantedAuthorities(BaseUser baseUser) {
        ArrayList<GrantedAuthority> list = new ArrayList<>();
        // 清除 Redis 中用户的角色
        roleTemplate.delete(baseUser.getId());
        for (String role : baseUser.getRoles()) {
            // 将用户的role，转换成权限信息
            list.add(new SimpleGrantedAuthority(role));
            // 存储角色到redis中
            roleTemplate.opsForList().rightPush(baseUser.getId(), role);
        }
        return list;
    }
}
